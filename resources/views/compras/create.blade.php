...
<div class="max-w-[1680px] mx-auto space-y-6" x-data="compraForm()">
    ...
    <form action="{{ route('compras.store') }}" method="POST" class="space-y-6">
        ...
        <!-- 2. Detalle de Artículos y Calculadora Reactiva (Alpine.js) -->
        <div class="bg-white rounded-2xl border border-slate-200 p-5 sm:p-6 shadow-sm space-y-4">
            <div class="flex items-center justify-between border-b border-slate-100 pb-3">
                ...
                <button type="button" @click="addItem()"
                    class="inline-flex items-center px-3 py-1.5 rounded-xl text-xs font-bold text-indigo-600 bg-indigo-50 hover:bg-indigo-100 transition">
            ...
            <!-- Tabla Reactiva Desktop -->
            <div class="hidden md:block overflow-x-auto">
                <table class="w-full divide-y divide-slate-200">
                    <thead class="bg-slate-50">
                        <tr>
                            <th class="px-3 py-3 text-left text-xs font-bold text-slate-500 uppercase tracking-wider w-5/12">Producto</th>
                    ...
                    <tbody class="divide-y divide-slate-100 bg-white">
                        <template x-for="(item, idx) in items" :key="idx">
                            <tr class="hover:bg-slate-50/60 transition">
                                <!-- Producto -->
                                <td class="px-3 py-3">
                                    <select :name="'items[' + idx + '][producto_id]'" x-model="item.producto_id" @change="onProductChange(idx)" required
            ...
            <!-- Vista Móvil de Artículos (Tarjetas reactivas para teléfonos) -->
            <div class="block md:hidden space-y-3">
                <template x-for="(item, idx) in items" :key="idx">
                    <div class="p-4 rounded-xl border border-slate-200 bg-slate-50/50 space-y-3">
                        <div class="flex items-center justify-between">
                            <span class="text-xs font-bold text-slate-500" x-text="'Ítem #' + (idx + 1)"></span>
                        ...
                        <div>
                            <label class="block text-xs font-bold text-slate-600 mb-1">Producto</label>
                            <select :name="'items[' + idx + '][producto_id]'" x-model="item.producto_id" @change="onProductChange(idx)" required
